...
plane1 = Plane("PLANE1 (BLUE)", image_plane1, pygame.math.Vector2(100, 300), 0, plane1_ai.plane_ai)
plane2 = Plane("PLANE2 (RED)", image_plane2, pygame.math.Vector2(700, 300), 180, plane2_ai.plane_ai)
...
while running:
    ...
    for plane in planes:
        ...
        plane.p.update(min(max(plane.p.x, 0), GAME_WIDTH), min(max(plane.p.y, 0), GAME_HEIGHT))

        if plane.shooting and plane.shoot_cooldown == 0:
            plane.shoot_cooldown = 60

            bullets.append(Bullet(plane.p + pygame.math.Vector2.from_polar((PLANE_SIZE + 5, -plane.r)), plane.r))
    ...
    bullets = [b for b in bullets if b.life > 0 and pygame.Rect(0, 0, GAME_WIDTH, GAME_HEIGHT).collidepoint(b.p.x, b.p.y)]
